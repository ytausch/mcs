...
def lsearch(fcn, x, f, f0, u, v, nf, stop, maxstep, gamma, hess, nsweep, nsweepbest):
    ...
    b = np.dot(np.abs(g).T, [max(abs(xmin[i]), abs(xold[i])) for i in
                             range(len(xmin))])  # print('chek b')  print(g,xmin,xold) # print(b)
    nstep = 0

    # print('ncall',nf, ncall, maxstep, len(ind),fmi, gain, r)
    while (ncall < nf) and (nstep < maxstep) and (
            (diag or len(ind) < n) or (stop[0] == 0 and fmi - gain <= stop[1]) or (b >= gamma * (f0 - f) and gain > 0)):
        # print('while')
        nstep += 1
