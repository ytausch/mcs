...
def initbox(x0, f0, l, L, istar, u, v, isplit, level, ipar, ichild, f, nboxes, prt):
    ...
    for i in range(n):
        ...
        for j in range(L[i]):
            ...
            nboxes += 1
            nchild += 1
            ipar[nboxes], level[nboxes], ichild[nboxes], f[0, nboxes] = genbox(par, level[par] + 3 - s, -nchild,
                                                                               f0[j + 1, i])
        # end for j
        if x0[i, L[i]] < v[i]:
            nboxes += 1
            nchild += 1
            ipar[nboxes], level[nboxes], ichild[nboxes], f[0, nboxes] = genbox(par, level[par] + 1, -nchild,
                                                                               f0[L[i], i])
