...
# %%------------------------------------------------------------------------------
def minqsub(nsub, free, L, dd, K, G, n, g, x, xo, xu, convex, xx, fct, nfree, unfix, alp, alpu, alpo, lba, uba, ier,
            subdone):
    ...
    alpu = max(
        [oo[i] for i in range(len(ind)) if pp[i] < 0] + [uu[i] for i in range(len(ind)) if pp[i] > 0] + [-np.inf])
